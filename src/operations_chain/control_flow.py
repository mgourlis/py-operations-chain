...
class IfElseOperation(ControlFlowOperation):
        ...
        return {
            ...
            'optional': {
                'else_branch': {
                    ...
                    'default': [],
                    'example': [{'operation': 'set', 'operation_config': {'value': 'N/A'}}]
                }
            }
        }
    ...
    async def direct_flow(self, value: Any, context: PipelineContext) -> Any:
        ...
        try:
            ...
            await PipelineExecutor(shared_data=context.shared_data).execute_pipeline(condition_ops, value)
            ...
            return await PipelineExecutor(shared_data=context.shared_data).execute_pipeline(then_ops, value)
        
        except ValidationError as e:
            # If ValidationError is caught, condition is "false"
            logger.debug(f"{self.name}: Condition failed. Executing 'else_branch'. Exception: {e}")
            ...
            return await PipelineExecutor(shared_data=context.shared_data).execute_pipeline(else_ops, value)
...
class ExecutePipelineOnPath(ControlFlowOperation):
    ...
    async def direct_flow(self, value: Any, context: PipelineContext) -> Any:
        ...
        if not isinstance(value, dict):
            raise ValueError(f"{self.name}: Input value must be a dictionary to access path '{path}'.")

        if not path or not sub_pipeline_def:
            raise ValueError(f"{self.name}: 'path' and 'pipeline' must be provided in the config.")
        ...
        try:
            ...
            raise ValueError(f"{self.name}: Path '{path}' does not exist in the input data.")

        # Execute the sub-pipeline on the extracted value
        logger.debug(f"{self.name}: Executing sub-pipeline on path '{path}'.")
        sub_ops = PipelineParser.from_json(sub_pipeline_def, f"{self.name}_sub_pipeline")
