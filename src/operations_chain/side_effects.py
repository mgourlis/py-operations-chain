...
class StoreInContextSideEffect(SideEffectOperation):
    ...
    async def perform(self, value: Any, context: PipelineContext) -> None:
        ...
        if not context_path:
            logger.warning(f"{self.name}: 'context_path' not specified in configuration.")
            return
        ...
        if value_path:
            if not isinstance(value, dict):
                logger.warning(f"{self.name}: 'value_path' is set, but input is not a dict.")
                return
            ...
            try:
                ...
                logger.warning(f"{self.name}: 'value_path' '{value_path}' not found in input.")
                return
        ...
        for key in keys[:-1]:
            ...
            if not isinstance(target_dict, dict):
                logger.warning(f"{self.name}: Context path '{context_path}' conflicts with non-dict value.")
                return

        final_key = keys[-1]
        
        if not overwrite and final_key in target_dict:
            logger.debug(f"{self.name}: Context path '{context_path}' exists and overwrite=false. Skipping.")
            return
