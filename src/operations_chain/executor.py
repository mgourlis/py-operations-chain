...
class PipelineExecutor:
    ...
    async def execute_pipeline(
        ...
        logger.debug(
            ...
            len(sorted_operations), type(initial_value).__name__
        )
        ...
        for step_index, operation_entity in enumerate(sorted_operations):
            ...
            try:
                operation = self.registry.get_operation(operation_name, operation_config)
            ...
            # Execute operation
            try:
                result = await operation.execute_with_metadata(current_value, self.context)
                current_value = result.value
                
                logger.debug(
                    "Operation %s completed in %.2fms",
                    operation_name, result.execution_time_ms
                )
                ...
                if is_required:
                    ...
                    if error_message:
                        raise ValidationError(error_message, operation_name=operation_name) from e
                    raise
                else:
                    logger.warning(
                ...
                if is_required:
                    msg = error_message or f"Operation {operation_name} failed: {str(e)}"
                    raise PipelineExecutionError(
                        msg,
                        step_index=step_index,
                        operation_name=operation_name,
