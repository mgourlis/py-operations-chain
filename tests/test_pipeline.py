...
class TestPipelineExecutor:
    ...
    async def test_pipeline_with_shared_data(self):
        pipeline = [
            OperationSpec(
                operation="store", operation_config={"context_path": "original"}
            ),
            OperationSpec(operation="uppercase"),
        ]
    ...
    async def test_non_required_operation_continues_on_error(self):
        pipeline = [
            ...
            OperationSpec(
                operation="default", operation_config={"default": "fallback"}
            ),
        ]
...
class TestExecutePipelineOnPath:
    ...
    async def test_transforms_nested_value(self, context):
        op = ExecutePipelineOnPath(
            name="test",
            config={
                "path": "user.name",
                "pipeline": [{"operation": "strip"}, {"operation": "uppercase"}],
            },
        )
