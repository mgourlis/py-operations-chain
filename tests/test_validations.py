...
class TestInListValidation:
    ...
    async def test_case_insensitive(self, context):
        op = InListValidation(name="test", config={
...
class TestNotInListValidation:
    ...
    async def test_passes_not_in_list(self, context):
        op = NotInListValidation(name="test", config={"forbidden_values": ["admin", "root"]})
...
class TestComparisonValidation:
    ...
    async def test_equal(self, context):
        op = ComparisonValidation(name="test", config={"operator": "eq", "compare_to": 10})
        result = await op.execute(10, context)
        assert result == 10
    
    @pytest.mark.asyncio
    async def test_greater_than(self, context):
        op = ComparisonValidation(name="test", config={"operator": "gt", "compare_to": 5})
        result = await op.execute(10, context)
        assert result == 10
    
    @pytest.mark.asyncio
    async def test_fails_comparison(self, context):
        op = ComparisonValidation(name="test", config={"operator": "gt", "compare_to": 10})
        with pytest.raises(ValidationError):
            await op.execute(5, context)
    ...
    async def test_compare_to_context(self, context):
        ...
        op = ComparisonValidation(name="test", config={
            "operator": "ge",
