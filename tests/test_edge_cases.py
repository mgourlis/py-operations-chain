...
class TestTransformationErrorHandling:
    ...
    async def test_on_error_return_none(self, context):
        op = TypeCastTransformation(
            name="test", config={"target_type": "int", "on_error": "return_none"}
        )
    ...
    async def test_on_error_return_original(self, context):
        op = JsonParseTransformation(
            name="test", config={"on_error": "return_original"}
        )
    ...
    async def test_json_serialize_non_serializable(self, context):
        ...
        op = JsonSerializeTransformation(
            name="test", config={"on_error": "return_original"}
        )
    ...
    async def test_replace_with_count(self, context):
        ...
        op = ReplaceTransformation(
            name="test", config={"search": "a", "replace": "X", "count": 1}
        )
...
class TestSideEffectEdgeCases:
    ...
    async def test_store_value_path_non_dict(self, context):
        ...
        op = StoreInContextSideEffect(
            name="test", config={"context_path": "result", "value_path": "field"}
        )
    ...
    async def test_store_value_path_missing(self, context):
        ...
        op = StoreInContextSideEffect(
            name="test",
            config={"context_path": "result", "value_path": "nonexistent.path"},
        )
    ...
    async def test_http_without_aiohttp(self, context):
        ...
        op = HttpRequestSideEffect(name="test", config={"url": "http://example.com"})
